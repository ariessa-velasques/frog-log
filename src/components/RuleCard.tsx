...
export default function RuleCard({ title, rules, type }: RuleCardProps) {
    const borderColor = type === 'do' ? 'border-pink-300' : 'border-red-300'
    const titleColor = type === 'do' ? 'text-pink-600' : 'text-red-500'
    const bgColor = type === 'do' ? 'bg-pink-50/60' : 'bg-red-50/60'
    const Icon = type === 'do' ? CheckCircle2 : XCircle
    const iconColor = type === 'do' ? 'text-pink-400' : 'text-red-400'
                ...
                <ul className="space-y-2">
                    {rules.map((rule) => (
                        <li key={rule.id} className="flex items-start gap-2 text-stone-600 text-sm">
                            <span className={`mt-1 ${type === 'do' ? 'text-pink-400' : 'text-red-400'}`}>•</span>
